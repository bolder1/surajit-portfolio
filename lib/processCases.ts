...
/**
 * Some engagements have more than one audience. Where `empathyMap` covers the
 * single operator a product is built for, `personas` covers the several people
 * a SYSTEM serves at once — each of whom would happily optimise it into
 * uselessness for the others. The `risk` line is the point: it names what
 * breaks if you serve only that person.
 */
export interface Persona {
  role: string;
  context: string;
  /** Verbatim — the sentence that gives away what they actually want. */
  says: string;
  needs: string;
  gets: string;
  risk: string;
...
/** One tier of the legacy store — what it holds fixed, what it frees up. */
...
/**
 * An instrument, not a reading. Adoption and velocity figures for the design
 * system are NDA'd, so what is published is the measurement model: what to
 * count, which direction is good, and what a bad reading actually means.
 * Publishing an invented number would be worse than publishing none.
 */
...
export interface ProcessCaseStudy {
  ...
  /* ── optional, for system engagements ─────────────────────────────
     A product case has one operator and a time compression. A system case
     has several audiences in tension, a legacy it must not break, and a
...
export const processCases: ProcessCaseStudy[] = [
...

  {
    ...
    oneLiner:
      "One definition behind five products — new work without redrawing what already shipped.",
    domain: "Design systems · Enterprise identity",
    year: "2024 — present",
    role: "Product Designer · miniOrange",
    deliverables: [
      "DRIFT AUDIT",
    ...
    hook: "Five products, already sold, already in daily use. The same blue existed in a dozen slightly different values, a dark mode meant redrawing a file, and a new brand meant a new library. Nobody had made a bad decision — five teams had each made a defensible local one.",
    pullQuote:
      "A system nobody adopts has exactly the same variable count as one everybody uses. The architecture is the easy half. The hard half is that five different people need it to do contradictory things.",
    compression: {
      theirs: "A DESIGN CYCLE PER SCREEN",
      mine: "A SPRINT TICKET",
      note: "The compression here is not a faster project. It is a screen that no longer needs a designer in the room.",
    },

    differentiator:
      "Most design systems ship as a library and a slide deck. This one ships as a token pipeline engineering builds against, a governance model that runs without me in it, and documentation rendered from the tokens it documents — so it is structurally incapable of describing a behaviour the system does not have.",

    problem: {
      headline: "Five products drifting apart, one defensible decision at a time",
      body: "The same logical component — a data table row — existed in a subtly different shape in every product, and each variant had a good local reason. That is what makes drift hard: no single decision is wrong, and the cumulative result was still the largest source of customer-reported inconsistency. Meanwhile every new screen re-litigated colour, spacing and radius from scratch, and every dark-mode or brand pass meant redrawing files by hand.",
    },
    constraint: {
      headline: "It had to describe what already shipped before proposing anything new",
      body: "Three constraints ruled out the obvious approach, which is to design the ideal system and migrate everyone onto it.",
      points: [
        "Five products were already sold and in daily operation. Anything that required them to be redrawn — or that cost a long-tenured administrator their muscle memory — would not have been adopted, whatever its merits.",
        "Strict universal components had already been tried and had failed. The system disagreed with each product's reality often enough that teams forked quietly, which is worse than open divergence because it cannot be measured.",
        "Three brands and two colour modes multiply. Treated as separate work that is six sets of screens; only a layered variable model turns it into one definition with modes on top.",
      ],
    },
    decision: {
      headline: "Built bottom-up, opened for use only once the semantic layer existed",
      body: "Each layer had to be stable before the one above could alias into it, so the order is not a preference — it is a dependency chain.",
      stages: [
        {
          title: "Audit what already exists",
          body: "Collect every colour, size and spacing value in circulation across the product files. The count is the argument for doing the work at all, and it becomes the baseline everything later is measured against.",
        },
        {
          title: "Primitives with no opinions",
          body: "Build the raw ramps and the pixel scale first. A primitive records what a value is, never where it is used — so naming stays mechanical and the shipped palette survives as data before any judgement is applied to it.",
        },
        {
          title: "Name roles, not values",
          body: "The semantic layer is where intent enters: surface, feedback, interactive. It is the only layer components may reference and the only one carrying light and dark — which is what lets a product team add a role instead of forking a component.",
        },
        {
          title: "Separate brand from colour mode",
          body: "Brand lives in its own collection with a mode per identity. Keeping it apart from light/dark is what makes three brands × two modes a multiplication rather than six sets of work.",
        },
        {
          title: "Make layout a variable",
          body: "Grid columns, gutter, margin, navigation width and the type scale each get a mode per breakpoint band. Responsive behaviour stops being a convention people remember and becomes something the file enforces.",
        },
        {
          title: "Ship readiness with the component",
          body: "Status is itself a component and the library is split by readiness, so “may I build against this?” is answered by where something lives rather than by finding the right person to ask.",
        },
      ],
    },
    outcome: {
      headline: "New products inherit; the old ones keep their faces",
      body: "A new product starts by selecting an existing theme mode and arrives already carrying the identity, spacing rhythm, type scale and both colour modes of everything before it. Divergence is still possible — it is just no longer accidental, because adding a token is a deliberate act that leaves a diff.",
      funnel: [
        { label: "Drift audit", note: "The count that justified doing the work at all" },
        { label: "One definition", note: "Six collections, resolving in a single direction" },
        { label: "Products onboarded", note: "Each one cheaper than the last, by design" },
        { label: "Screens shipped unassisted", note: "Built from the library with no designer on the ticket" },
      ],
      metrics: [
        { label: "Definition → outcome", value: "763 variables → 6,443 components" },
    ...
    personas: {
      headline: "Five people, one file, opposing interests",
      body: "A design system has no single user. It has several, and each would happily optimise it into uselessness for the others. The architecture is not a preference — it is the shape that falls out of serving all five at once.",
      people: [
        {
          role: "The product designer, on one of five products",
          context: "Has a screen to draw this week that the library does not quite cover, and a deadline shorter than the time it takes to get the library changed.",
          says: "The system doesn't know about my product's reality.",
          needs: "To extend the system without waiting for it, and without quietly forking it.",
          gets: "The semantic layer. They introduce a role, not a value — and every component referencing that role absorbs it at once.",
          risk: "Serve only this person and the library forks five ways inside two quarters. That is the failure the audit found, not a hypothetical.",
        },
        {
          role: "The frontend engineer, with no designer on the ticket",
          context: "A module is due this sprint and there is no design behind it. They are not trying to make a visual decision; they are trying to avoid having to.",
          says: "Just tell me which one to use.",
          needs: "Defaults that are already the reviewed answer, so building the ordinary thing requires no taste.",
          gets: "Component properties whose options are token names. Padding offers spacing steps, not pixels — the wrong value is not in the dropdown.",
          risk: "Serve only this person and the system freezes into a kit. Nothing new can enter it, and design goes back to happening outside the file.",
        },
        {
          role: "The product manager, selling the suite",
          context: "Five products, sold together, demoed in the same hour. Every visual disagreement between them is a question they have to answer in the room.",
          says: "Why does the table look different in that product?",
          needs: "Family resemblance across the suite without commissioning a redesign programme to get it.",
          gets: "Brand as a mode rather than a library. Three brands cost 46 tokens.",
          risk: "Serve only this person and everything converges on the lowest common denominator — five products that agree with each other and fit none of their users.",
        },
        {
          role: "The long-tenured administrator",
          context: "Runs these consoles every day and has done for years. Was not asked about any of this, and is the only person here who does not care that a design system exists.",
          says: "I don't want to relearn where things are.",
          needs: "The product they already know how to operate — not an improved version that costs them a week.",
          gets: "Primitives holding the shipped values under a stable name. Modernising changes what a token points at; it does not move the control they reach for.",
          risk: "Serve only this person and nothing ever changes. Their interest is legitimate and it is also the one that ossifies a system if it goes unchallenged.",
        },
        {
          role: "The next maintainer",
          context: "Inherits hundreds of variables and dozens of component sets they did not name, and has to make a decision on day one without the person who made the original one.",
          says: "Why is it like this?",
          needs: "Intent recoverable from the file itself, because the alternative is a document nobody updated.",
          gets: "Mechanical names at the primitive layer, roles at the semantic layer, readiness shipped as a component — and a published issue list rather than a tidy one.",
          risk: "Serve only this person and you optimise for documentation over shipping, and the system becomes a museum of its own decisions.",
        },
      ],
    },

    vault: {
      headline: "New products without redrawing the old ones",
      body: "The system was not introduced to an empty company. Anything requiring the shipped products to be redrawn would have been refused, so it had to be able to describe what already existed before it was allowed to propose anything new. Each layer fixes one thing and frees another.",
      layers: [
        {
          name: "Primitives",
          preserves: "The exact values the products shipped with, under stable mechanical names",
          frees: "Nothing — this layer is deliberately inert, which is what makes it trustworthy",
        },
        {
          name: "Aliased scales",
          preserves: "The spacing and radius rhythm of the existing layouts",
          frees: "The base unit, which ripples through every scale at once",
        },
        {
          name: "Semantic roles",
          preserves: "Where a control lives and what it means",
          frees: "What it looks like, per colour mode and per brand",
        },
        {
          name: "Theme modes",
          preserves: "Each product's established identity",
          frees: "Which identity a new product adopts on day one",
        },
      ],
      cost: "The honest half: capturing what exists means capturing its mistakes. Misspellings and a handful of tokens whose names disagree with their values were carried forward intact, because the audit recorded reality rather than an improved version of it. Preservation and correction pull against each other — this system errs toward preservation, which is defensible while products are migrating and indefensible afterwards. That makes it a dated decision, not a permanent one.",
    },

    scaleModel: {
      headline: "How a system like this is proved to scale",
      body: "Variable counts describe size, not success — a system nobody adopted has the same count as one everybody uses. What follows is the instrument set rather than the readings: adoption and velocity figures here are under NDA, and a number invented to fill a column is worse than an empty one.",
      metrics: [
        {
          name: "Insertion rate",
          instrument: "Library analytics, per component per month",
          good: "Climbs as each product onboards, then plateaus",
          bad: "Flat after onboarding means teams are drawing instead of placing, and the component they need is missing",
        },
        {
          name: "Detach rate",
          instrument: "Detached instances ÷ total instances",
          good: "Falls after each release and stays low",
          bad: "A rising rate names the component lacking a variant somebody needed — every detach is a feature request written in the wrong place",
        },
        {
          name: "Hardcoded values",
          instrument: "Static analysis of the codebase for raw values where a token exists",
          good: "Falls as the token pipeline lands in each repo",
          bad: "Rising while the variable count grows means the code side never adopted, and the system is design-only",
        },
        {
          name: "Time to first screen, unassisted",
          instrument: "One engineer, one real screen, library only, timed each quarter",
          good: "Shortens, and the transcript shows no aesthetic decisions",
          bad: "A long time points at a missing default, which is far cheaper to fix than a missing component",
        },
        {
          name: "Handoff clarifications",
          instrument: "Design questions per feature in the engineering channel",
          good: "Falls toward zero for patterns the library already solves",
          bad: "A repeated question names an undocumented decision precisely — answer it in the file, not the thread",
        },
        {
          name: "Contrast + naming gates",
          instrument: "Automated checks over every semantic pair and every numeric token name",
          good: "Zero, enforced as a gate rather than reported as a finding",
          bad: "Anything above zero is a defect the system can reintroduce at will, which is the definition of an ungoverned system",
        },
      ],
      curve: "One number decides whether this is a system or a well-organised library: the onboarding cost of every product, recorded in the order they arrive. A flat curve means each product pays full price. A falling curve means the nth product is cheaper than the one before it, because the semantic layer already holds the roles it needs — and that decline, not the variable count, is the whole argument for having built it.",
    },

    journeyMap: {
      persona: "Frontend engineer, new screen, no designer allocated",
      stages: [
        { stage: "Ticket lands", emotion: -1, note: "A module is due this sprint and there is no design behind it." },
        { stage: "Falls back on guessing", emotion: -2, note: "Copies a colour by eye from a neighbouring screen and hopes nobody files a bug." },
        { stage: "Opens the library", emotion: 0, note: "Only components marked ready are publishable, so what is reachable is already what is safe." },
        { stage: "Sets properties", emotion: 1, note: "Padding offers spacing steps, not pixels — the wrong value is not in the dropdown." },
        { stage: "Resizes the screen", emotion: 1, note: "Breakpoint bands carry the layout; the screen was only ever drawn once." },
        { stage: "Ships it", emotion: 2, note: "Dark mode, brand and contrast were resolved before the screen existed." },
      ],
    },
    empathyMap: {
      persona: "Frontend engineer, no designer on the ticket",
      says: ["“Just tell me which one to use.”", "“Is this component safe to build against?”"],
      thinks: [
        "“I'm not trying to make a visual decision — I'm trying to avoid having to.”",
        "“If I guess wrong it ships, and then it's a bug with my name on it.”",
      ],
      does: [
        "Copies values from a neighbouring screen when nothing else is available",
        "Asks in the channel and waits for someone to answer",
      ],
      feels: [
        "Reluctant to make a call that was never theirs to make",
        "Noticeably faster when the correct option is simply the default",
      ],
    },
    userFlow: {
      before: [
        { label: "Select the theme mode once, at the top" },
        { label: "Place a component marked ready" },
        { label: "Set properties from token options" },
      ],
      branch: {
        question: "Does the pattern already exist?",
        yes: "Ship — layout follows the bands",
        no: "Enters intake as a use case",
      },
      after: "Two products needing it is the bar for library work",
    },
  },
];
